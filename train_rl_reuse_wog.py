...
from td3_fixed_wog import TD3Agent, TD3ObsAgent, TD3ObsActAgent
...
def reuse_and_freeze_modules(agent, ckpt_dir, device):
    ...
    * 注意：不要冻结/替换 gripper 的优化器；gripper 一直可训练。
    """
    if not ckpt_dir:
    ...
    # 4) 禁用自监督分支 + 替换优化器（**不替换 gripper 的优化器**）
    ...
    # 5) 打印可训练参数（若有 gripper，应该仍可训练）
    def ntrainable(mod):
        return sum(p.numel() for p in mod.parameters() if p.requires_grad)
    report = {}
    for name in ['actor','critic','obs_enc','obs_dec','act_enc','act_dec','inv_dyn','fwd_dyn','grip_head']:
        if hasattr(agent, name):
...
def main():
    ...
    agent_kwargs = dict(
        ...
        # 新增：gripper 的学习率（默认 1e-3）
        grip_lr=float(params.get('grip_lr', 1e-3)),
    )
    ...
    for step in range(params['total_timesteps']):
        ...
        # act
        action = agent.sample_action(obs)  # 默认带探索噪声（夹爪不加噪）
