...
def _eef_lin_ang_jacobians(env, site_id: int):
    ...
    for ep in tqdm(episodes, desc=f"[{robots}] build condition (in-memory)"):
        ...
        for k in cond_plan.appended_order:
            if k == C_KEY_SENTINEL:
                ...
                V = np.zeros((T, 6), dtype=np.float32)  # [v(3), w(3)]
                for t in range(T):
                    _set_qpos_and_forward(base_env, q_for[t])
                    Jp, Jr = _eef_lin_ang_jacobians(base_env, site_id)   # (3,nv), (3,nv)
                    v = (Jp[:, :q_for.shape[1]] @ a_arm[t]).astype(np.float64)  # 线速度
                    w = (Jr[:, :q_for.shape[1]] @ a_arm[t]).astype(np.float64)  # 角速度
                    V[t, :3] = v.astype(np.float32)
                    V[t, 3:] = w.astype(np.float32)
...
def main():
    ...
    TASKS = ["Lift", "Reach", "Stack"]  # 强制三任务；Stack 与 Reach 同等纳入训练
    ...
    # )
    print("[prep] building augmented episodes (SRC+TGT across Lift/Reach/Stack) ...")
