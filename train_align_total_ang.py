...
# - 【修改】派生键目前支持：
#       - "robot0_eef_pos"  (EEF 世界系位置，3维)      ← 如需启用，请在 DERIVED_KEYS 打开
#       - "eef_action_vel"  (EEF 线/角速度，6维=v3+w3，v=Jp(q)@a_arm, w=Jr(q)@a_arm)
...
DERIVED_KEYS = {
    # 如需支持 EEF 世界位置，请解除下面一行的注释
    # "robot0_eef_pos": 3,
    # EEF 线/角速度 [v(3), w(3)]，由关节动作 a_arm 通过雅可比得到，总 6 维
    "eef_action_vel": 6,
}

# ------- Reach 兄弟目录与有效性检测 -------
def _dir_has_npz(d: pathlib.Path) -> bool:
...
def _derive_reach_brother_dir(base_dir: str | pathlib.Path) -> pathlib.Path | None:
    """把路径里的 'Lift' 替换为 'Reach'，若存在且有 .npz 则返回，否则 None。"""
    base = pathlib.Path(base_dir)
    cand = pathlib.Path(str(base).replace("Lift", "Reach"))
    if _dir_has_npz(cand):
        return cand
    return None
...
def main():
    ...
    # === 先装载主任务（通常是 Lift） ===
    episodes_src_aug_lift = _precompute_cond_and_augment_episodes_from_dir(
        episodes_dir=params['src_buffer'],
        obs_keys_for_loading=src_train_obs_keys,                      # 仅机器人段 + 原生 cond
        env_name=params['env_name'],                                  # 主任务（Lift）
        robots=params['src_env']['robot'],
        controller_type=params['src_env']['controller_type'],
        env_kwargs=params.get('env_kwargs', {}),                       # Lift 可用 env_kwargs
        robot_len=src_robot_len,
        act_arm_dim=src_act_arm_dim,
        cond_keys_native=cond_keys_native,
        cond_keys_derived=cond_keys_derived,
        tqdm_desc="[SRC][Lift] append derived cond"
    )
    episodes_tgt_aug_lift = _precompute_cond_and_augment_episodes_from_dir(
        ...
        cond_keys_native=cond_keys_native,
        cond_keys_derived=cond_keys_derived,
        tqdm_desc="[TGT][Lift] append derived cond"
    )

    # === 兄弟目录替换：把 Lift -> Reach ===
    src_reach_dir = _derive_reach_brother_dir(params['src_buffer'])
    tgt_reach_dir = _derive_reach_brother_dir(params['tgt_buffer'])

    if (src_reach_dir is None) or (tgt_reach_dir is None):
        raise RuntimeError(
            f"[Reach] 兄弟目录缺失或无 .npz："
            f"src_reach_dir={src_reach_dir}, tgt_reach_dir={tgt_reach_dir}"
        )

    # === Reach 装载：只读 robot_obs_keys + cond.keys(native)；不传 env_kwargs ===
    episodes_src_aug_reach = _precompute_cond_and_augment_episodes_from_dir(
        episodes_dir=str(src_reach_dir),
        obs_keys_for_loading=src_train_obs_keys,                      # 严格：机器人段 + 原生 cond
        env_name="Reach",                                             # 强制任务为 Reach
        robots=params['src_env']['robot'],
        controller_type=params['src_env']['controller_type'],
    ...
    episodes_tgt_aug_reach = _precompute_cond_and_augment_episodes_from_dir(
        episodes_dir=str(tgt_reach_dir),
        obs_keys_for_loading=tgt_train_obs_keys,
        env_name="Reach",                                             # 强制任务为 Reach
        robots=params['tgt_env']['robot'],
        controller_type=params['tgt_env']['controller_type'],
    ...
    # === 合并 Lift + Reach 两类 episode ===
    episodes_src_aug = episodes_src_aug_lift + episodes_src_aug_reach
    episodes_tgt_aug = episodes_tgt_aug_lift + episodes_tgt_aug_reach

    # （可选）在这里做一次维度严查：所有 episode 的 obs 最后一维一致
    def _check_obs_lastdim(demos: list, expected_dim: int, side: str):
        for i, ep in enumerate(demos[:10]):  # 抽样前 10 条足够发现问题
            _d = int(ep['obs'].shape[-1])
            if _d != expected_dim:
                raise RuntimeError(f"[{side}] episode[{i}] obs_dim={_d} != expected_dim={expected_dim}")

    _check_obs_lastdim(episodes_src_aug, src_env.observation_space.shape[0] + derived_total_dim, "SRC")
    _check_obs_lastdim(episodes_tgt_aug, tgt_env.observation_space.shape[0] + derived_total_dim, "TGT")
