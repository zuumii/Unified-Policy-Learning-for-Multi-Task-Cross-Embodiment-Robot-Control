...
# === PATCH START: add GripperHead ===
class GripperHead(nn.Module):
    """
    只吃 obj_obs 的夹爪网络，tanh 输出 ∈[-1,1]。
    结构：n_layers 个 256 隐藏层 + 1 输出层（即常说的 3×256 就是 n_layers=3）
    """
    def __init__(self, obj_dim: int, hidden: int = 256, n_layers: int = 3):
        super().__init__()
        # 关键修改：隐藏层 = n_layers，而不是 (n_layers - 1)
        dims = [obj_dim] + [hidden] * n_layers + [1]
    ...
    def forward(self, obj_obs: torch.Tensor) -> torch.Tensor:
        return torch.tanh(self.net(obj_obs))
# === PATCH END ===
...
# === PATCH START: Agent grip_head lifecycle ===
class Agent:
    ...
    def __init__(self, obs_dims, act_dims, device):
        ...
        # 独立 gripper head（默认没有；用 init_grip_head 开启）
        self.grip_head: Optional[GripperHead] = None
    ...
    def freeze(self):
        ...
        if self.grip_head is not None:
            for p in self.grip_head.parameters():
                p.requires_grad = False

    def init_grip_head(self, enabled: bool, trainable: bool, obj_dim: int, hidden: int = 256, n_layers: int = 3):
        """由外部（train_align）调用，按 config 初始化夹爪头"""
        if not enabled:
            self.grip_head = None
            return
        self.grip_head = GripperHead(obj_dim=obj_dim, hidden=hidden, n_layers=n_layers).to(self.device)
        if not trainable:
            for p in self.grip_head.parameters():
                p.requires_grad = False

    def try_load_grip_head(self, model_dir: pathlib.Path, fname: str = "grip_head.pt") -> bool:
        """若存在 grip_head.pt，则严格加载；失败则跳过"""
        if self.grip_head is None:
            return False
        path = pathlib.Path(model_dir) / fname
        if not path.exists():
            print(f"[align] grip_head ckpt not found: {path}")
            return False
        sd = torch.load(path, map_location=self.device)
        try:
            self.grip_head.load_state_dict(sd, strict=True)
            print(f"[align] grip_head loaded from {path}")
            return True
        except Exception as e:
            print(f"[align] grip_head strict load failed, skip: {e}")
            return False
# === PATCH END ===
    ...
    # === PATCH START: Agent._rebuild_actor_to_match_ckpt(force_out_dim) ===
    def _rebuild_actor_to_match_ckpt(self, actor_sd: dict, force_out_dim: Optional[int] = None):
        in_dim, ckpt_out, n_layers, hidden_dim = self._infer_actor_arch_from_state_dict(actor_sd)
        out_dim = int(force_out_dim) if (force_out_dim is not None) else ckpt_out
        print(f"[align] Rebuild Actor to match ckpt: in={in_dim}, out={out_dim}, hidden={hidden_dim}, layers={n_layers}"
            f"{' (force_out_dim)' if (force_out_dim is not None) else ''}")
...
class ObsAgent(Agent):
    ...
    # === PATCH START: ObsAgent.save append grip_head ===
    ...
    # === PATCH START: ObsAgent.load try grip_head ===
    def load(self, model_dir):
        ...
        # 若存在 grip_head.pt 且已构建 grip_head，则尝试加载
        grip_path = pathlib.Path(model_dir) / "grip_head.pt"
        if grip_path.exists() and (self.grip_head is not None):
            self.try_load_grip_head(model_dir)
    # === PATCH END ===
...
class ObsActAgent(ObsAgent):
    def __init__(self, obs_dims, act_dims, device,
        ...
        # === PATCH START: ObsActAgent actor out_dim = lat_act_dim (no grip) ===
        a_n = actor_n_layers or n_layers
        a_h = actor_hidden_dim or hidden_dim
        old_actor = self.actor
        # 只输出 arm latent（lat_act_dim），不含 grip
    ...
    # === PATCH START: ObsActAgent.load_actor force out_dim ===
    def load_actor(self, model_dir):
        model_dir = pathlib.Path(model_dir)
        actor_sd = torch.load(model_dir / "actor.pt", map_location=self.device)
        try:
            # 强制重建为 “只出 arm latent”（lat_act_dim）
            self._rebuild_actor_to_match_ckpt(actor_sd, force_out_dim=self.lat_act_dim)
            # 宽松加载：除最后一层行数变化外，其它层严格复用
            self.actor.load_state_dict(actor_sd, strict=False)
        except RuntimeError as e:
            print(f"[align.load_actor] rebuild+load failed: {e}")
            raise
        for p in self.actor.parameters():
            p.requires_grad = False
    # === PATCH END ===

    # === PATCH START: ObsActAgent.sample_action with grip_head ===
    def sample_action(self, obs, deterministic=False):
        with torch.no_grad():
            ...
            # 只用 arm latent（actor 已解耦为无 grip 输出）
            lat_arm = self.actor(torch.cat([z, obj_obs], dim=-1))
            arm = self.act_dec(torch.cat([robot_obs, lat_arm], dim=-1))
            # grip 独立 head
            if self.grip_head is not None:
                grip = self.grip_head(obj_obs)
            else:
                grip = torch.zeros(arm.shape[0], 1, device=arm.device)
            act = torch.cat([arm, grip], dim=-1)
...
class DiffusionPriors:
    ...
    def load_from_dir(self, cfg: Dict[str, Any], cond_dim: int, src_robot_dim: int, tgt_robot_dim: int, lat_dim: int):
        ...
        _lambda_cfg = cfg.get("lambda", 1.0)
        if isinstance(_lambda_cfg, (int, float)):
            self.lambda_lat = self.lambda_src = self.lambda_tgt = float(_lambda_cfg)
        else:
            self.lambda_lat = float(_lambda_cfg.get("lat", 1.0))
            self.lambda_src = float(_lambda_cfg.get("src", 1.0))
            self.lambda_tgt = float(_lambda_cfg.get("tgt", 1.0))
        ...
        self.lat_prior = _load_one("score_lat_act", lat_dim)
        self.src_prior = _load_one("score_src_joint", src_robot_dim)
        self.tgt_prior = _load_one("score_tgt_joint", tgt_robot_dim)
        ...
        src_mu    = norm["src_joint"]["mu"]; src_std    = norm["src_joint"]["std"]
        tgt_mu    = norm["tgt_joint"]["mu"]; tgt_std    = norm["tgt_joint"]["std"]
        ...
        if self.cond_dim > 0:
            cond_src_mu = norm["cond"]["mu_src"]; cond_src_std = norm["cond"]["std_src"]
            cond_tgt_mu = norm["cond"]["mu_tgt"]; cond_tgt_std = norm["cond"]["std_tgt"]        
    ...
    def _safe_cat(self, parts, dim: int = 1):
    ...
    # ---------- loss (替换原三個 loss_*) ----------
    def loss_lat(self, obs: torch.Tensor | None, act: torch.Tensor | None, c: Optional[torch.Tensor]) -> torch.Tensor:
        x = self._safe_cat([obs, act], dim=1)   # 直接 obs ⊕ act，不做降维
        return self._loss_one(self.lat_prior, x, c)

    def loss_src(self, obs: torch.Tensor | None, act: torch.Tensor | None, c: Optional[torch.Tensor]) -> torch.Tensor:
        x = self._safe_cat([obs, act], dim=1)
        return self._loss_one(self.src_prior, x, c)

    def loss_tgt(self, obs: torch.Tensor | None, act: torch.Tensor | None, c: Optional[torch.Tensor]) -> torch.Tensor:
        x = self._safe_cat([obs, act], dim=1)
        return self._loss_one(self.tgt_prior, x, c)
...
# === FLOW PATCH START: Flow priors base & bundles (obs / act) ===
class _PriorBundleBase:
    """
    与 DiffusionPriors / ActionDiffusionPriors 对齐的统一接口：
    - 属性：enabled, lambda_lat/src/tgt, cond_dim, cond_keys, name
    - 方法：load_from_dir(...), loss_lat(...), loss_src(...), loss_tgt(...)
    注意：内部参数一律 requires_grad=False + eval()，forward 保留梯度回传到生成器侧。
    """
    name: str = "flow"
    ...
    # --- 归一化缓存，与 Diffusion 完全同名 ---
    @staticmethod
    def _read_norm_yaml(root: pathlib.Path):
        from ruamel.yaml import YAML
        return YAML(typ='safe').load(open(root / "stats" / "normalization.yml", "r"))

    @staticmethod
    def _to_tensor(x, like: torch.Tensor):
        return torch.as_tensor(x, dtype=like.dtype, device=like.device)

    def _norm_x(self, prior, x: torch.Tensor):
        return (x - prior.mu_x) / (prior.std_x + 1e-8)

    def _norm_c(self, prior, c: torch.Tensor | None):
        if (prior is None) or (prior.c_dim == 0) or (c is None):
            return None
        return (c - prior.mu_c) / (prior.std_c + 1e-8)

    # ---- 统一加载入口（由子类实现具体统计字段名与 x_dim）----
    def load_from_dir(self, cfg: Dict[str, Any], **dims):
        raise NotImplementedError

    # ---- 三路损失（由子类实现）----
    def loss_lat(self, x_like: torch.Tensor, c_like: torch.Tensor | None) -> torch.Tensor:
        raise NotImplementedError

    def loss_src(self, x_like: torch.Tensor, c_like: torch.Tensor | None) -> torch.Tensor:
        raise NotImplementedError

    def loss_tgt(self, x_like: torch.Tensor, c_like: torch.Tensor | None) -> torch.Tensor:
        raise NotImplementedError


class _FlowPriorModel(nn.Module):
    def __init__(self, x_dim: int, c_dim: int, t_dim: int, hidden: int = 256, n_layers: int = 3, mode: str = "fm"):
        super().__init__()
        self.x_dim = x_dim
        self.c_dim = c_dim
        self.mode = mode
        self.t_dim = t_dim
        self.time_embed = (SinusoidalTimeEmbedding(t_dim) if mode == "fm" else None)
        in_dim = x_dim + (c_dim if c_dim > 0 else 0) + (t_dim if mode == "fm" else 0)
        self.net = utils.build_mlp(
            in_dim, (x_dim if mode == "fm" else 1),
            n_layers, hidden, activation='relu', output_activation='identity', batch_norm=False
        )
        # buffers...
        self.register_buffer('mu_x', torch.zeros(x_dim))
        self.register_buffer('std_x', torch.ones(x_dim))
        self.register_buffer('mu_c', torch.zeros(max(c_dim, 1)))
        self.register_buffer('std_c', torch.ones(max(c_dim, 1)))

    def forward(self, x: torch.Tensor, c: torch.Tensor | None, t: torch.Tensor | None):
        if self.mode == "fm":
            assert t is not None and self.time_embed is not None
            if (c is None) or (self.c_dim == 0):
                inp = torch.cat([x, self.time_embed(t)], dim=-1)
            else:
                inp = torch.cat([x, c, self.time_embed(t)], dim=-1)
            return self.net(inp)
        else:
            inp = x if (c is None or self.c_dim == 0) else torch.cat([x, c], dim=-1)
            return self.net(inp)


class FlowPriors(_PriorBundleBase):
    """
    OBS 分支的 Flow 先验（lat/src/tgt），接口与 DiffusionPriors 等形。
    cfg 关键字段：
      - enabled, lambda.{lat,src,tgt}, cond_dim, cond_keys
      - priors.dir, priors.use_ema, priors.mode: "fm" | "nf"
    """
    name = "flow"

    def __init__(self, device: torch.device):
        super().__init__(device)
        self.mode = "fm"    # 缺省以 Flow Matching 形式运行
        self.T = 128        # 仅 FM 用于 t 采样；NF 可忽略

    def _load_one(self, root: pathlib.Path, fname: str, x_dim: int, c_dim: int) -> _FlowPriorModel:
        path = root / "models" / fname
        if not path.exists():
            raise RuntimeError(f"[flow] 模型不存在: {path}")
        model = _FlowPriorModel(x_dim=x_dim, c_dim=c_dim, t_dim=128, hidden=512, n_layers=4, mode=self.mode).to(self.device)
        sd = torch.load(path, map_location=self.device)
        try:
            model.load_state_dict(sd, strict=True)
        except RuntimeError:
            # 如需支持“按 ckpt 重建宽深”，可仿照 _rebuild_prior_to_match_ckpt；此处最小实现直接 strict=False
    ...
    def load_from_dir(self, cfg: Dict[str, Any], *, cond_dim: int, src_robot_dim: int, tgt_robot_dim: int, lat_dim: int):
        ...
        root = pathlib.Path(pri_cfg.get("dir", ""))
        use_ema = bool(pri_cfg.get("use_ema", True))
        self.mode = str(pri_cfg.get("mode", "fm")).lower()
        if self.enabled and ((not root.exists()) or (not (root / "models").exists())):
            raise RuntimeError(f"[flow] priors.dir 不存在或无 models 子目录: {root}")
        if cond_dim != self.cond_dim:
            print(f"[flow] warn: cond_dim from align={cond_dim}, bundle={self.cond_dim}")

        if not self.enabled:
            print("[flow] disabled by config.")
            return

        # 装载三路
        def _fname(base): return f"{base}_ema.pt" if use_ema else f"{base}.pt"
        self.lat_prior = self._load_one(root, _fname("flow_lat"),      lat_dim,        self.cond_dim)
        self.src_prior = self._load_one(root, _fname("flow_src_obs"),  src_robot_dim,  self.cond_dim)
        self.tgt_prior = self._load_one(root, _fname("flow_tgt_obs"),  tgt_robot_dim,  self.cond_dim)

        # 写归一化
        norm = self._read_norm_yaml(root)
        latent_mu, latent_std = norm["latent"]["mu"],  norm["latent"]["std"]
        src_mu,    src_std    = norm["src_obs"]["mu"], norm["src_obs"]["std"]
        tgt_mu,    tgt_std    = norm["tgt_obs"]["mu"], norm["tgt_obs"]["std"]
        cond_src_mu, cond_src_std = norm["cond_src"]["mu"], norm["cond_src"]["std"]
        cond_tgt_mu, cond_tgt_std = norm["cond_tgt"]["mu"], norm["cond_tgt"]["std"]

        # 长度强校验
        if len(latent_mu) != self.lat_prior.x_dim or len(latent_std) != self.lat_prior.x_dim:
            raise RuntimeError("[flow] latent 统计长度不等于 lat_dim")
        if len(src_mu) != self.src_prior.x_dim or len(src_std) != self.src_prior.x_dim:
            raise RuntimeError("[flow] src_obs 统计长度不等于 src_robot_dim")
        if len(tgt_mu) != self.tgt_prior.x_dim or len(tgt_std) != self.tgt_prior.x_dim:
            raise RuntimeError("[flow] tgt_obs 统计长度不等于 tgt_robot_dim")
        if self.cond_dim > 0:
            if len(cond_src_mu) != self.cond_dim or len(cond_src_std) != self.cond_dim:
                raise RuntimeError("[flow] cond_src 统计长度不等于 cond_dim")
            if len(cond_tgt_mu) != self.cond_dim or len(cond_tgt_std) != self.cond_dim:
                raise RuntimeError("[flow] cond_tgt 统计长度不等于 cond_dim")

        # 回写 buffer（lat/src 用 cond_src；tgt 用 cond_tgt，与 diffusion 对齐）
        for prior, mu, std in [(self.lat_prior, latent_mu, latent_std),
                               (self.src_prior, src_mu, src_std),
                               (self.tgt_prior, tgt_mu, tgt_std)]:
            prior.mu_x.data.copy_(self._to_tensor(mu, prior.mu_x))
            prior.std_x.data.copy_(self._to_tensor(std, prior.std_x))
        if self.cond_dim > 0:
            self.lat_prior.mu_c.data.copy_(self._to_tensor(cond_src_mu, self.lat_prior.mu_c))
            self.lat_prior.std_c.data.copy_(self._to_tensor(cond_src_std, self.lat_prior.std_c))
            self.src_prior.mu_c.data.copy_(self._to_tensor(cond_src_mu, self.src_prior.mu_c))
            self.src_prior.std_c.data.copy_(self._to_tensor(cond_src_std, self.src_prior.std_c))
            self.tgt_prior.mu_c.data.copy_(self._to_tensor(cond_tgt_mu, self.tgt_prior.mu_c))
            self.tgt_prior.std_c.data.copy_(self._to_tensor(cond_tgt_std, self.tgt_prior.std_c))
        def _peek(tag, p):
            print(f"[{tag}] x_dim={p.x_dim}, c_dim={p.c_dim}, "
                f"mu_x.mean={p.mu_x.mean().item():.4g}, std_x.mean={p.std_x.mean().item():.4g}, "
                f"mu_c.mean={(p.mu_c.mean().item() if p.c_dim>0 else float('nan')):.4g}, "
                f"std_c.mean={(p.std_c.mean().item() if p.c_dim>0 else float('nan')):.4g}")

        _peek("flow:lat", self.lat_prior)
        _peek("flow:src", self.src_prior)
        _peek("flow:tgt", self.tgt_prior)
        
        print(f"[flow] loaded from {root} | mode={self.mode} | cond_dim={self.cond_dim}, keys={self.cond_keys}")

    # ---- 三路损失：FM 用“速度场回归 MSE”；NF 用“负对数似然” ----
    def _loss_fm(self, prior: _FlowPriorModel, x: torch.Tensor, c: torch.Tensor | None) -> torch.Tensor:
        B = x.shape[0]
        t = torch.full((B,), 0.5, device=x.device, dtype=x.dtype)
        # 归一化到 prior 统计
        x_n = self._norm_x(prior, x)
        c_n = self._norm_c(prior, c)
        # 简化：用“噪声方向”当作目标速度（与 diffusion 同阶的 proxy，保持等价正则角色）
        noise = torch.randn_like(x_n)
        v_hat = prior(x_n, c_n, t)
        return F.mse_loss(v_hat, noise)
        # return (v_hat ** 2).mean()

    def _loss_nf(self, prior: _FlowPriorModel, x: torch.Tensor, c: torch.Tensor | None) -> torch.Tensor:
        x_n = self._norm_x(prior, x)
        c_n = self._norm_c(prior, c)
        logp = prior(x_n, c_n, t=None)  # [B,1]
        return (-logp).mean()

    def _loss_one(self, prior, x, c):
        if prior is None:
            return torch.tensor(0.0, device=self.device)
        if self.mode == "fm":
            return self._loss_fm(prior, x, c)
        else:
            return self._loss_nf(prior, x, c)

    def loss_lat(self, z_tgt: torch.Tensor, c_tgt: torch.Tensor | None) -> torch.Tensor:
        return self._loss_one(self.lat_prior, z_tgt, c_tgt)

    def loss_src(self, fake_src_robot: torch.Tensor, c_tgt: torch.Tensor | None) -> torch.Tensor:
        return self._loss_one(self.src_prior, fake_src_robot, c_tgt)

    def loss_tgt(self, real_tgt_robot: torch.Tensor, c_src: torch.Tensor | None) -> torch.Tensor:
        return self._loss_one(self.tgt_prior, real_tgt_robot, c_src)


class ActionFlowPriors(FlowPriors):
    """
    ACT 分支的 Flow 先验（lat_act/src_act/tgt_act），字段名改用 latent_act / src_act / tgt_act。
    """
    name = "flow-act"

    def load_from_dir(self, cfg: Dict[str, Any], *, cond_dim: int, src_act_dim: int, tgt_act_dim: int, lat_act_dim: int):
        self.enabled = bool(cfg.get("enabled", False))
        self.lambda_lat = float(((cfg.get("lambda") or {}).get("lat", 1.0)))
        self.lambda_src = float(((cfg.get("lambda") or {}).get("src", 1.0)))
        self.lambda_tgt = float(((cfg.get("lambda") or {}).get("tgt", 1.0)))
        ...
        self.mode = str(pri_cfg.get("mode", "fm")).lower()
        if self.enabled and ((not root.exists()) or (not (root / "models").exists())):
            raise RuntimeError(f"[flow-act] priors.dir 不存在或无 models 子目录: {root}")
        if cond_dim != self.cond_dim:
            print(f"[flow-act] warn: cond_dim from align={cond_dim}, bundle={self.cond_dim}")

        if not self.enabled:
            print("[flow-act] disabled by config.")
            return

        def _fname(base): return f"{base}_ema.pt" if use_ema else f"{base}.pt"
        self.lat_prior = self._load_one(root, _fname("flow_lat_act"), lat_act_dim, self.cond_dim)
        self.src_prior = self._load_one(root, _fname("flow_src_act"), src_act_dim, self.cond_dim)
        self.tgt_prior = self._load_one(root, _fname("flow_tgt_act"), tgt_act_dim, self.cond_dim)

        # 写归一化（动作字段）
        norm = self._read_norm_yaml(root)
        latent_mu, latent_std = norm["latent_act"]["mu"], norm["latent_act"]["std"]
        src_mu,    src_std    = norm["src_act"]["mu"],    norm["src_act"]["std"]
        tgt_mu,    tgt_std    = norm["tgt_act"]["mu"],    norm["tgt_act"]["std"]
        cond_src_mu, cond_src_std = norm["cond_src"]["mu"], norm["cond_src"]["std"]
        cond_tgt_mu, cond_tgt_std = norm["cond_tgt"]["mu"], norm["cond_tgt"]["std"]

        if len(latent_mu) != self.lat_prior.x_dim or len(latent_std) != self.lat_prior.x_dim:
            raise RuntimeError("[flow-act] latent_act 统计长度不等于 lat_act_dim")
        if len(src_mu) != self.src_prior.x_dim or len(src_std) != self.src_prior.x_dim:
            raise RuntimeError("[flow-act] src_act 统计长度不等于 src_act_dim")
        if len(tgt_mu) != self.tgt_prior.x_dim or len(tgt_std) != self.tgt_prior.x_dim:
            raise RuntimeError("[flow-act] tgt_act 统计长度不等于 tgt_act_dim")
        if self.cond_dim > 0:
            if len(cond_src_mu) != self.cond_dim or len(cond_src_std) != self.cond_dim:
                raise RuntimeError("[flow-act] cond_src 统计长度不等于 cond_dim")
            if len(cond_tgt_mu) != self.cond_dim or len(cond_tgt_std) != self.cond_dim:
                raise RuntimeError("[flow-act] cond_tgt 统计长度不等于 cond_dim")
        ...
        _peek("flow-act:lat", self.lat_prior)
        _peek("flow-act:src", self.src_prior)
        _peek("flow-act:tgt", self.tgt_prior)

        print(f"[flow-act] loaded from {root} | mode={self.mode}")
# === FLOW PATCH END ===
...
class ObsAligner:
    def __init__(
        ...
        flow_bundle_cfg: Optional[Dict[str, Any]] = None,
        ...
        # === FLOW PATCH START: obs prior bundles container & loading ===
        # 统一改成“列表”收集 obs 先验（支持同时开 diffusion 与 flow）
        self.obs_prior_bundles = []



        # # 打印组合
        # if len(self.obs_prior_bundles) == 0:
        #     print("[DEBUG] No OBS prior bundle enabled.")
        # else:
        #     names = ", ".join([getattr(b, "name", type(b).__name__) 
        #            for b in self.obs_prior_bundles 
        #            if getattr(b, "enabled", False)])
        #     print(f"[DEBUG] OBS prior bundles enabled: {names}")
        # # === FLOW PATCH END ===
    ...
    def update_gen(self, src_obs, src_act, src_next_obs, tgt_obs, tgt_act, tgt_next_obs, L, step,
        ...
        # === FLOW PATCH START: aggregate OBS priors (diffusion + flow, any combo) ===
        obs_prior_loss_total = torch.tensor(0.0, device=self.device)
        if hasattr(self, "obs_prior_bundles"):
            for B in self.obs_prior_bundles:
                if (B is None) or (not getattr(B, "enabled", False)):
                    continue
                L_lat = B.loss_lat(fake_lat_obs, c_tgt)
                L_src = B.loss_src(fake_src_obs, c_tgt)
                L_tgt = B.loss_tgt(real_tgt_obs, c_src)
                bundle_loss = (B.lambda_lat * L_lat + B.lambda_src * L_src + B.lambda_tgt * L_tgt)
                obs_prior_loss_total = obs_prior_loss_total + bundle_loss

                # 可分开记曲线：train_prior/{B.name}/lat|src|tgt|total
                if step % self.log_freq == 0:
                    L.add_scalar(f'train_prior/{B.name}/lat',   L_lat.item(), step)
                    L.add_scalar(f'train_prior/{B.name}/src',   L_src.item(), step)
                    L.add_scalar(f'train_prior/{B.name}/tgt',   L_tgt.item(), step)
                    L.add_scalar(f'train_prior/{B.name}/total', bundle_loss.item(), step)
        # === FLOW PATCH END ===    

        # 总损失（保持原名项权重不变，再加 diffusion）
        loss = gen_loss + self.lmbd_cyc * cycle_loss + self.lmbd_dyn * (inv_loss + fwd_loss) + obs_prior_loss_total
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        # 统一记录 OBS 侧先验（可能有多个 bundle：diffusion / flow）
        d["priors_obs_meta"] = []
        if hasattr(self, "obs_prior_bundles"):
            for B in self.obs_prior_bundles:
                if B is None:
                    continue
                d["priors_obs_meta"].append({
                    "name": getattr(B, "name", "unknown"),
                    "enabled": bool(getattr(B, "enabled", False)),
                    "lambda_lat": float(getattr(B, "lambda_lat", 0.0)),
                    "lambda_src": float(getattr(B, "lambda_src", 0.0)),
                    "lambda_tgt": float(getattr(B, "lambda_tgt", 0.0)),
                    "cond_dim": int(getattr(B, "cond_dim", 0)),
                })

        # 兼容老字段（非必须）：保留一份“旧版 priors_meta”，用 getattr 防止 self.priors 不存在时报错
        pri = getattr(self, "priors", None)
        d["priors_meta"] = {
            "enabled": (pri is not None and getattr(pri, "enabled", False)),
            "lambda_lat": (0.0 if pri is None else float(getattr(pri, "lambda_lat", 0.0))),
            "lambda_src": (0.0 if pri is None else float(getattr(pri, "lambda_src", 0.0))),
            "lambda_tgt": (0.0 if pri is None else float(getattr(pri, "lambda_tgt", 0.0))),
        }

        # 其它开关（GAN、SEW 等如需也可写 meta，这里保持你的原逻辑即可）
        d["gan"] = {
...
class ObsActAligner(ObsAligner):
    def __init__(
        ...
        flow_bundle_cfg: Optional[Dict[str, Any]] = None,
        flow_act_bundle_cfg: Optional[Dict[str, Any]] = None,
        cycle_lambda: float = 10.0,
        dynamics_lambda: float = 10.0,
        sew_cfg: Optional[Dict[str, Any]] = None,
        ...
        # === FLOW PATCH START: ACT prior bundles container & loading ===
        # 1) obs 侧：父类里已经创建了 self.obs_prior_bundles
        # 2) act 侧：自己创建列表
        self.act_prior_bundles = []

        # 兼容旧：如果传了 diffusion_bundle_cfg，则保留原有 self.priors 行为，但也 push 到列表
        if diffusion_bundle_cfg is not None:
            ...
            self.priors.load_from_dir(
                ...
                src_robot_dim=self.src_obs_dim + self.src_act_dim,
                tgt_robot_dim=self.tgt_obs_dim + self.tgt_act_dim,
                lat_dim=self.lat_obs_dim + self.lat_act_dim
            )
            self.obs_prior_bundles.append(self.priors)

        # 新增：Flow（obs）
        # if flow_bundle_cfg is not None:
        #     self.priors_flow = FlowPriors(self.device)
        #     self.priors_flow.load_from_dir(
        #         cfg=flow_bundle_cfg,
        #         cond_dim=int(flow_bundle_cfg.get("cond_dim", 0)),
        #         src_robot_dim=self.src_obs_dim,
        #         tgt_robot_dim=self.tgt_obs_dim,
        #         lat_dim=self.lat_obs_dim
        #     )
        #     self.obs_prior_bundles.append(self.priors_flow)

        # 已有的动作 Diffusion（保持你的原始逻辑不删，只是也 append 进列表）
        ...
        #         )
        #     self.act_prior_bundles.append(self.priors_act)

        # 新增：动作 Flow
        # if flow_act_bundle_cfg is not None:
        #     self.priors_act_flow = ActionFlowPriors(self.device)
        #     self.priors_act_flow.load_from_dir(
        #         cfg=flow_act_bundle_cfg,
        #         cond_dim=int(flow_act_bundle_cfg.get("cond_dim", 0)),
        #         src_act_dim=self.src_act_dim,
        #         tgt_act_dim=self.tgt_act_dim,
        #         lat_act_dim=self.lat_act_dim
            # )
        # # === 立刻查看 enabled 是否被置上 ===
        #     print("[DIAG][flow-obs] after load:",
        #         "name=", getattr(self.priors_flow, "name", type(self.priors_flow).__name__),
        #         "enabled=", getattr(self.priors_flow, "enabled", None),
        #         "cond_dim=", getattr(self.priors_flow, "cond_dim", None))
            # self.obs_prior_bundles.append(self.priors_flow)            
            # self.act_prior_bundles.append(self.priors_act_flow)

        # # 日志
        # if len(self.act_prior_bundles) == 0:
        #     print("[DEBUG] No ACT prior bundle enabled.")
        # else:
        #     names = ", ".join([b.name for b in self.act_prior_bundles if getattr(b, "enabled", False)])
        #     print(f"[DEBUG] ACT prior bundles enabled: {names}")
        # === FLOW PATCH END ===   
        ...
        if pri is None:
            ...
            print(f"[DEBUG] Diffusion enabled = {pri.enabled}, "
                f"λ = ({pri.lambda_lat}, {pri.lambda_src}, {pri.lambda_tgt}), "
                f"cond_dim = {pri.cond_dim}")      
            
                # ---- Diffusion-Act (act) ----
        pri_a = getattr(self, "priors_act", None)
        if pri_a is None:
            print("[DEBUG] Diffusion-Act priors = None (disabled)")
        else:
            # 兼容两种写法：单独属性 或 字典
            if all(hasattr(pri_a, k) for k in ("lambda_lat", "lambda_src", "lambda_tgt")):
                lam_lat, lam_src, lam_tgt = pri_a.lambda_lat, pri_a.lambda_src, pri_a.lambda_tgt
            else:
                lam_dict = getattr(pri_a, "lambda", None) or getattr(pri_a, "lmbd", None) or getattr(pri_a, "lambda_dict", None) or {}
                lam_lat = lam_dict.get("lat", "NA")
                lam_src = lam_dict.get("src", "NA")
                lam_tgt = lam_dict.get("tgt", "NA")
            print(f"[DEBUG] Diffusion-Act enabled = {getattr(pri_a, 'enabled', False)}, "
                  f"λ = ({lam_lat}, {lam_src}, {lam_tgt}), "
                  f"cond_dim = {getattr(pri_a, 'cond_dim', 'NA')}")
            
                # ---- Flow (obs) ----
        flo = getattr(self, "priors_flow", None)
        if flo is None:
            print("[DEBUG] Flow priors = None (disabled)")
        else:
            # 兼容两种写法：属性 或 字典
            if all(hasattr(flo, k) for k in ("lambda_lat", "lambda_src", "lambda_tgt")):
                lam_lat, lam_src, lam_tgt = flo.lambda_lat, flo.lambda_src, flo.lambda_tgt
            else:
                lam_dict = getattr(flo, "lambda", None) or getattr(flo, "lmbd", None) or getattr(flo, "lambda_dict", None) or {}
                lam_lat = lam_dict.get("lat", "NA")
                lam_src = lam_dict.get("src", "NA")
                lam_tgt = lam_dict.get("tgt", "NA")
            print(f"[DEBUG] Flow enabled = {getattr(flo, 'enabled', False)}, "
                  f"λ = ({lam_lat}, {lam_src}, {lam_tgt}), "
                  f"cond_dim = {getattr(flo, 'cond_dim', 'NA')}")

        # ---- Flow-Act (act) ----
        flo_a = getattr(self, "priors_act_flow", None)
        if flo_a is None:
            print("[DEBUG] Flow-Act priors = None (disabled)")
        else:
            if all(hasattr(flo_a, k) for k in ("lambda_lat", "lambda_src", "lambda_tgt")):
                lam_lat, lam_src, lam_tgt = flo_a.lambda_lat, flo_a.lambda_src, flo_a.lambda_tgt
            else:
                lam_dict = getattr(flo_a, "lambda", None) or getattr(flo_a, "lmbd", None) or getattr(flo_a, "lambda_dict", None) or {}
                lam_lat = lam_dict.get("lat", "NA")
                lam_src = lam_dict.get("src", "NA")
                lam_tgt = lam_dict.get("tgt", "NA")
            print(f"[DEBUG] Flow-Act enabled = {getattr(flo_a, 'enabled', False)}, "
                  f"λ = ({lam_lat}, {lam_src}, {lam_tgt}), "
                  f"cond_dim = {getattr(flo_a, 'cond_dim', 'NA')}")

        # （可选）总览当前已挂载的先验 bundle 名称，便于确认是否加入了 update 流程
        try:
            obs_names = [getattr(b, "name", "?") for b in getattr(self, "obs_prior_bundles", [])]
            act_names = [getattr(b, "name", "?") for b in getattr(self, "act_prior_bundles", [])]
            # print(f"[DEBUG] OBS prior bundles enabled: {', '.join(obs_names) if obs_names else '(none)'}")
            # print(f"[DEBUG] ACT prior bundles enabled: {', '.join(act_names) if act_names else '(none)'}")
        except Exception as _e:
            print(f"[DEBUG] prior bundles overview failed: {_e}") 
        # 目标侧动作分支也参与优化
    ...
    def update_gen(self, src_obs, src_act, src_next_obs, tgt_obs, tgt_act, tgt_next_obs, L, step,
        ...
        # === FLOW PATCH START: OBS priors aggregate ===
        obs_prior_loss_total = torch.tensor(0.0, device=self.device)
        if hasattr(self, "obs_prior_bundles"):
            for B in self.obs_prior_bundles:
                if (B is None) or (not getattr(B, "enabled", False)):
                    continue
                L_lat = B.loss_lat(fake_lat_obs, fake_lat_act, c_tgt)
                L_src = B.loss_src(fake_src_obs, fake_src_act,c_tgt)
                L_tgt = B.loss_tgt(real_tgt_obs, real_tgt_act, c_src)
                bundle_loss = (B.lambda_lat * L_lat + B.lambda_src * L_src + B.lambda_tgt * L_tgt)
                obs_prior_loss_total = obs_prior_loss_total + bundle_loss
                if step % self.log_freq == 0:
                    L.add_scalar(f'train_prior/{B.name}/lat',   L_lat.item(), step)
        ...
        # # === FLOW PATCH START: ACT priors aggregate ===
        # act_prior_loss_total = torch.tensor(0.0, device=self.device)
        # if hasattr(self, "act_prior_bundles"):
        #     for B in self.act_prior_bundles:
        #         if (B is None) or (not getattr(B, "enabled", False)):
        #             continue
        #         L_lat_a = B.loss_lat(fake_lat_act, c_tgt_act)
        #         L_src_a = B.loss_src(fake_src_act, c_tgt_act)
        #         L_tgt_a = B.loss_tgt(real_tgt_act, c_src_act)
        #         bundle_loss_a = (B.lambda_lat * L_lat_a + B.lambda_src * L_src_a + B.lambda_tgt * L_tgt_a)
        #         act_prior_loss_total = act_prior_loss_total + bundle_loss_a
        #         if step % self.log_freq == 0:
        #             L.add_scalar(f'train_prior_act/{B.name}/lat',   L_lat_a.item(), step)
        #             L.add_scalar(f'train_prior_act/{B.name}/src',   L_src_a.item(), step)
        #             L.add_scalar(f'train_prior_act/{B.name}/tgt',   L_tgt_a.item(), step)
        #             L.add_scalar(f'train_prior_act/{B.name}/total', bundle_loss_a.item(), step)
        # === FLOW PATCH END ===

        # 总损失（追加 act 先验）
        loss = gen_loss + self.lmbd_cyc * cycle_loss + self.lmbd_dyn * (inv_loss + fwd_loss) \
        + obs_prior_loss_total 
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        # 记录 ACT diffusion 旧字段（兼容用）
        pri_act = getattr(self, "priors_act", None)
        base["priors_act_meta"] = {
            "enabled": (pri_act is not None and getattr(pri_act, "enabled", False)),
            "lambda_lat": (0.0 if pri_act is None else float(getattr(pri_act, "lambda_lat", 0.0))),
            "lambda_src": (0.0 if pri_act is None else float(getattr(pri_act, "lambda_src", 0.0))),
            "lambda_tgt": (0.0 if pri_act is None else float(getattr(pri_act, "lambda_tgt", 0.0))),
        }

        # 统一记录 OBS 先验（父类里已经写了 priors_obs_meta，这里不重复）
        # 追加：ACT 侧统一列表（允许 flow + diffusion）
        base["priors_act_bundles_meta"] = []
        if hasattr(self, "act_prior_bundles"):
            for B in self.act_prior_bundles:
                if B is None:
                    continue
                base["priors_act_bundles_meta"].append({
                    "name": getattr(B, "name", "unknown"),
                    "enabled": bool(getattr(B, "enabled", False)),
                    "lambda_lat": float(getattr(B, "lambda_lat", 0.0)),
                    "lambda_src": float(getattr(B, "lambda_src", 0.0)),
