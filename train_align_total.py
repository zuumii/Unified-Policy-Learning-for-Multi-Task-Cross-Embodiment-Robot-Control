...
# - 【修改】派生键目前支持：
#       - "robot0_eef_pos"  (EEF 世界系位置，3维)  # 目前未启用，如需可在 DERIVED_KEYS 打开
...
DERIVED_KEYS = {
    # "robot0_eef_pos": 3,  # 如需启用 EEF 世界坐标，打开此行并在 cond.keys 中加入
    "eef_action_vel": 3,
}
...
def _precompute_cond_and_augment_episodes_from_dir(
    ...
    for ep in pbar:
        ...
        # EEF 线速度
        if need_vel:
            v_curr = np.zeros((T, 3), dtype=np.float32)
            for t in range(T):
                Jp, _ = _eef_jacobians(base_env, q_traj[t], robot_id=0)
                v_curr[t] = (Jp @ a_arm[t]).astype(np.float32)
            v_next = np.vstack([v_curr[1:], v_curr[-1:]]) if T > 1 else v_curr.copy()
        ...
        for k in cond_keys_derived:
            if k == "robot0_eef_pos":
                ...
                derived_curr_parts.append(v_curr)
                derived_next_parts.append(v_next)
            else:
...
def main():
    ...
    # ===【保持】只保留一套 cond：严格按 config.cond.keys 的顺序拼接 ===
    ...
    # 构造训练/eval/probe 环境（与原逻辑一致） —— 主任务：Lift
    src_env = utils.make(
        params['env_name'],
    ...
    # probe 用 robosuite 原生字典观测检测 shape（Lift）
    ...
    if strict_dim_check and native_dim_src != native_dim_tgt:
        raise RuntimeError(f"[cond(native)][Lift] 维度在 src/tgt 不一致：src={native_dim_src}, tgt={native_dim_tgt}")
    ...
    # ------------------------------
    # Replay buffers（统一预计算派生 cond 后装载） —— 主任务 Lift
    # ------------------------------
    ...
    # 统一预计算（Lift）派生 cond，并将其追加到 obs 尾部
    ...
    # 追加了派生段后的 obs_shape = env.observation_space + sum(derived dims) —— 对应 Lift 的 shape
    ...
    # === Reach 数据并入（兄弟目录替换 + 严查 + 预计算 + 追加入 buffer）===
    # 1) 兄弟目录替换（Lift -> Reach）
    src_reach_buffer = pathlib.Path(str(params['src_buffer']).replace("Lift", "Reach"))
    tgt_reach_buffer = pathlib.Path(str(params['tgt_buffer']).replace("Lift", "Reach"))

    def _has_episode_files(d: pathlib.Path) -> bool:
        if not d.exists() or not d.is_dir():
            return False
        # 仅判断是否存在 .npz（utils.load_episodes 默认支持）
        has_npz = any(d.glob("*.npz")) or any(d.glob("**/*.npz"))
        return bool(has_npz)

    if not (_has_episode_files(src_reach_buffer) and _has_episode_files(tgt_reach_buffer)):
        raise RuntimeError(f"[Reach] 数据缺失：请检查 {src_reach_buffer} 与 {tgt_reach_buffer} 是否存在且包含 .npz")

    # 2) Reach probe：只检查 robot 段与 cond(native) 维度，严禁传 env_kwargs
    src_reach_probe_env = utils.make_robosuite_env(
        "Reach",
        robots=params['src_env']['robot'],
        controller_type=params['src_env']['controller_type'],
    )
    tgt_reach_probe_env = utils.make_robosuite_env(
        "Reach",
        robots=params['tgt_env']['robot'],
        controller_type=params['tgt_env']['controller_type'],
    )
    src_reach_probe = safe_reset(src_reach_probe_env, max_tries=50, reseed=True)
    tgt_reach_probe = safe_reset(tgt_reach_probe_env, max_tries=50, reseed=True)

    # Robot 段长度必须与 Lift 一致（同一机器人）
    src_reach_robot_len = int(np.concatenate([src_reach_probe[k] for k in params['src_env']['robot_obs_keys']]).shape[0])
    tgt_reach_robot_len = int(np.concatenate([tgt_reach_probe[k] for k in params['tgt_env']['robot_obs_keys']]).shape[0])
    if src_reach_robot_len != src_robot_obs_shape[0]:
        raise RuntimeError(f"[Reach] src robot_obs_dim 与 Lift 不一致：reach={src_reach_robot_len}, lift={src_robot_obs_shape[0]}")
    if tgt_reach_robot_len != tgt_robot_obs_shape[0]:
        raise RuntimeError(f"[Reach] tgt robot_obs_dim 与 Lift 不一致：reach={tgt_reach_robot_len}, lift={tgt_robot_obs_shape[0]}")

    # 原生 cond 维度也必须一致
    reach_native_dim_src = _shape_of_keys(src_reach_probe, cond_keys_native)
    reach_native_dim_tgt = _shape_of_keys(tgt_reach_probe, cond_keys_native)
    if reach_native_dim_src != native_dim_src:
        raise RuntimeError(f"[Reach] src cond(native) 维度与 Lift 不一致：reach={reach_native_dim_src}, lift={native_dim_src}")
    if reach_native_dim_tgt != native_dim_tgt:
        raise RuntimeError(f"[Reach] tgt cond(native) 维度与 Lift 不一致：reach={reach_native_dim_tgt}, lift={native_dim_tgt}")

    # 3) Reach 的派生 cond 预计算 + 追加（严格：env_kwargs 传 {}）
    episodes_src_aug_reach = _precompute_cond_and_augment_episodes_from_dir(
        episodes_dir=src_reach_buffer,
        obs_keys_for_loading=src_train_obs_keys,      # 只读 robot + native-cond
        env_name="Reach",
        robots=params['src_env']['robot'],
        controller_type=params['src_env']['controller_type'],
        env_kwargs={},                                # 不带 env_kwargs
        robot_len=src_robot_len,                      # 与 Lift 对齐
        act_arm_dim=src_act_arm_dim,
        cond_keys_native=cond_keys_native,
        cond_keys_derived=cond_keys_derived,
        tqdm_desc="[SRC][Reach] append derived cond"
    )
    episodes_tgt_aug_reach = _precompute_cond_and_augment_episodes_from_dir(
        episodes_dir=tgt_reach_buffer,
        obs_keys_for_loading=tgt_train_obs_keys,
        env_name="Reach",
        robots=params['tgt_env']['robot'],
        controller_type=params['tgt_env']['controller_type'],
        ...
        cond_keys_native=cond_keys_native,
        cond_keys_derived=cond_keys_derived,
        tqdm_desc="[TGT][Reach] append derived cond"
    )

    # 4) Reach 数据装前校验：obs/action 维度与切片合法性
    def _validate_augmented_episodes(eps: list, expected_obs_dim: int, expected_action_dim: int,
                                     cond_slices: dict, keys_in_order: list, tag: str):
        for i, ep in enumerate(eps[:10]):  # 抽样检查前 10 条，足够发现结构问题
            obs_full = ep['obs']
            act = ep['action'] if 'action' in ep else ep.get('actions', None)
            if act is None:
                act = ep.get('act', None)
            if act is None:
                raise RuntimeError(f"[{tag}] episode[{i}] 缺少动作字段(action/actions/act)")

            if obs_full.shape[1] != expected_obs_dim:
                raise RuntimeError(f"[{tag}] episode[{i}] obs_full_dim={obs_full.shape[1]} != expected={expected_obs_dim}")

            if act.shape[1] != expected_action_dim:
                raise RuntimeError(f"[{tag}] episode[{i}] action_dim={act.shape[1]} != expected={expected_action_dim}")

            # 切片合法性（基于 Lift 建立的 slices）
            for k in keys_in_order:
                s, e = cond_slices[k]
                if not (0 <= s < e <= obs_full.shape[1]):
                    raise RuntimeError(f"[{tag}] episode[{i}] cond slice 越界: key={k}, slice=({s},{e}), obs_dim={obs_full.shape[1]}")

    _validate_augmented_episodes(
        episodes_src_aug_reach, expected_obs_dim=src_obs_shape[0],
        expected_action_dim=src_env.action_space.shape[0],
        cond_slices=src_cond_slices, keys_in_order=cond_keys_all, tag="SRC-Reach"
    )
    _validate_augmented_episodes(
        episodes_tgt_aug_reach, expected_obs_dim=tgt_obs_shape[0],
        expected_action_dim=tgt_env.action_space.shape[0],
        cond_slices=tgt_cond_slices, keys_in_order=cond_keys_all, tag="TGT-Reach"
    )

    # 5) Reach 直接追加装入现有 buffer（训练循环零改动）
    src_buffer.add_rollouts(episodes_src_aug_reach)
    tgt_buffer.add_rollouts(episodes_tgt_aug_reach)
    ...
    # ------------------------------
    # 训练循环（buffer.obs = Lift + Reach；判别器/评估保持不变，只评估 Lift）
    # ------------------------------
