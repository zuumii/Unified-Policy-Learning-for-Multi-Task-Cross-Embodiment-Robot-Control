...
# Condition = EEF linear velocity v = Jp(q) @ qdot
# - qdot 直接用“关节动作（臂部维）”代替（不做维度检测）
# - v 在线计算，不落盘；把 v 追加到 obs / next_obs 尾部
# - 训练流程与日志、统计保持不变
# =========================================================
...
# ---- constants ----
V_KEY = "cond_lin_vel"
V_DIM = 3  # EEF linear velocity: x, y, z
...
def probe_dims(env_name: str, robot: str, controller: str, env_kwargs: Dict[str, Any],
    ...
    cond_dim = (V_DIM if len(cond_keys) > 0 else 0)
...
def build_env_for_buffer(env_name: str, robot: str, controller: str, env_kwargs: Dict[str, Any],
    ...
    # 在线增广，所以此处只让实时环境暴露 robot_obs_keys（不包含 V_KEY）
...
def _compute_v_from_actions(env, q_traj: np.ndarray, a_arm_traj: np.ndarray, site_id: int) -> np.ndarray:
    """
    q_traj:     [T, nq]
    ...
    for t in range(T):
        ...
        V[t] = (Jp[:, :nq] @ a_arm_traj[t]).astype(np.float32)
    return V
...
def precompute_augmented_in_memory_using_actions(
    ...
    在线用 action(臂部维) 当作 qdot 计算 v=Jp(q)@qdot，
    把 v 追加到 obs/next_obs 尾部，不落盘，直接返回增广后的 episodes 列表。
    """
    ...
    out_eps = []
    for ep in tqdm(episodes, desc=f"[{robots}] augment with v=Jp(q)*action (in-memory)"):
        # 恢复 q
        ...
        a_arm = action[:, :act_arm_dim]     # [L_raw-1, nq]（按你要求，不做检测）

        # 用 q[:-1] 与 action 对齐，让 v[t] 对应 obs[t]、action[t]
        T_trans = action.shape[0]
        q_for_v = q[:T_trans]               # [T_trans, nq]

        # 在线求 v，长度 == T_trans
        v_full = _compute_v_from_actions(base_env, q_for_v, a_arm, site_id)  # [T_trans, 3]

        # 构造 obs_aug / next_obs_aug，三者统一用 T = min(...)
        T = min(T_trans, ep['obs'].shape[0], ep['next_obs'].shape[0])

        # 追加 v 到尾部（obs 用 v[t]；next_obs 用 v[t+1]，末尾用复制 v[T-1] 兜底）
        v_curr = v_full[:T]  # [T, 3]
        if T > 1:
            v_next = np.vstack([v_full[1:T], v_full[T-1:T]])  # [T, 3]
        else:
            v_next = v_curr.copy()

        obs_aug = np.concatenate([ep['obs'][:T], v_curr], axis=-1)
        next_obs_aug = np.concatenate([ep['next_obs'][:T], v_next], axis=-1)

        new_ep = dict(ep)
        new_ep['obs'] = obs_aug
        new_ep['next_obs'] = next_obs_aug
        new_ep[V_KEY] = v_curr  # 可选：保留调试
    ...
    for _ in pbar:
        ...
        if cfg.schedule_type == 'ddpm':
            ...
            if cfg.loss_weight in ('sigma2', 'snr'):
                # 注意：这里 betas[t_idx] 仅在你需要用到时再传入；当前未用到 betas
...
def main():
    ...
    print("[prep] augmenting in-memory with v = Jp(q) * action (SRC/TGT) ...")
    ...
    # --- 用增广后的 episodes 构建 buffers（obs 已包含 v 在尾部）
    ...
    normalization = {
        'cond_src':   {'key': V_KEY, 'mu': mu_c_src.tolist(), 'std': std_c_src.tolist()},
        'cond_tgt':   {'key': V_KEY, 'mu': mu_c_tgt.tolist(), 'std': std_c_tgt.tolist()},
    ...
    prior_lat_act = PriorModel(x_dim=lat_act_dim, c_dim=V_DIM, hidden=hidden_dim,
    ...
    prior_src_act = PriorModel(x_dim=src_act_arm_dim, c_dim=V_DIM,
    ...
    prior_tgt_act = PriorModel(x_dim=tgt_act_arm_dim, c_dim=V_DIM,
                               hidden=hidden_dim, n_layers=n_layers,
                               use_condition=use_condition, time_embed_dim=time_embed_dim,
                               mode=schedule_type).to(device)
