...
# - 修订：act-cond 固定为 c = [p(3), v(3)]（EEF 世界系位置 + 线速度，共 6 维）
# - 修订：J@a 与 p 的批量计算可选 tqdm 进度显示
# =========================================================
import pathlib
import argparse
import time
...
# ------- act-cond = [p(3), v(3)] 的总开关与参数 -------
USE_PV_AS_ACT_COND = True        # ← 开：act 条件固定采用 c = [p(3), v(3)]
COND_ACT_DIM_FIXED = 6           # ← 恒为 6 维
PV_TQDM_EVERY = 200              # ← 每隔多少个 step 显示一次 tqdm（1=每步都显示）
PV_TQDM_DESC = "EEF [p,v] (act-cond)"   # ← tqdm 标题

# ------- 是否在装载数据时预计算 [p,v] 并追加到 obs 尾部 -------
USE_PRECOMPUTE_PV = True         # ← 打开：离线把 [p,v] 算好塞进 buffer
PV_DIM = 6                       # ← 3(位置) + 3(线速度)
# 当 USE_PRECOMPUTE_PV=True 时，obs 实际长度 = 原env.observation_space + PV_DIM
# 训练环路里直接从 obs 尾部切片拿 act-cond（[p,v]），不再每步调用雅可比
...
# ---- NEW: 逐 episode 预计算 [p,v] 并把它们追加到 obs / next_obs 尾部 ----
...
def _compute_pv_traj_from_actions(env, q_traj: np.ndarray, a_arm_traj: np.ndarray, robot_id: int = 0):
    """
    q_traj:     [T, dof_arm]   -- 关节角轨迹（臂部）
    a_arm_traj: [T, dof_arm]   -- 关节动作（把 arm 部分当作 qdot）
    return:     (p_traj, v_traj)，分别为 [T,3] 的 EEF 世界系位置 与 线速度
    """
    T, _ = q_traj.shape
    p_out = np.zeros((T, 3), dtype=np.float32)
    v_out = np.zeros((T, 3), dtype=np.float32)
    for t in range(T):
        # 位置
        p_out[t] = _eef_pos(env, q_traj[t], robot_id=robot_id)
        # 线速度
        Jp, _ = _eef_jacobians(env, q_traj[t], robot_id=robot_id)  # (3,dof)
        v_out[t] = (Jp @ a_arm_traj[t]).astype(np.float32)
    return p_out, v_out


def _precompute_pv_and_augment_episodes_from_dir(
    ...
    tqdm_desc: str = "[precompute] -> append [p,v]"
):
    """
    读取 episodes_dir 下的原始 *.npz（保持你训练时的 obs_keys 拼接顺序）,
    在线计算 [p,v]（p = EEF 世界系位置，v = Jp(q)@a），
    然后把 [p,v] 追加到 obs / next_obs 尾部（不落盘），返回增广后的 episodes 列表。
    """
    episodes_dir = pathlib.Path(episodes_dir)

    # robosuite 原生 env，用于雅可比/位置
    ...
    # 使用“训练时的 obs_keys”（robot + merged_cond），保证后续切 cond 的逻辑不变
    ...
    for ep in pbar:
        ...
        # 计算 p,v 及其 next
        p_curr, v_curr = _compute_pv_traj_from_actions(base_env, q_traj, a_arm, robot_id=0)  # [T,3], [T,3]
        if T > 1:
            p_next = np.vstack([p_curr[1:], p_curr[-1:]])
            v_next = np.vstack([v_curr[1:], v_curr[-1:]])
        else:
            p_next = p_curr.copy()
            v_next = v_curr.copy()

        pv_curr = np.concatenate([p_curr, v_curr], axis=-1)  # [T,6]
        pv_next = np.concatenate([p_next, v_next], axis=-1)  # [T,6]

        # 追加到 obs 尾部（保持其它段顺序不变）
        obs_aug      = np.concatenate([obs, pv_curr], axis=-1)
        next_obs_aug = np.concatenate([next_obs, pv_next], axis=-1)
        ...
        # 便于调试
        new_ep['cond_eef_p']  = p_curr
        new_ep['cond_eef_v']  = v_curr
        out.append(new_ep)
...
def _eval_reach_final_dist(env, agent, episodes: int = 4):
    """
    仅用于 env_name == 'Reach' 的快速评估：
    - 跑 episodes 条完整 episode（不保存视频）
    - 对每条 episode 的“最后一帧”从 obs dict 读取 target_to_robot0_eef_pos
      若不存在该键，退化为 ||target_pos - robot0_eef_pos||
    - 返回 (mean, var) —— 方差用总体方差 np.var
    """
    dists = []
    ...
    return float(dists.mean()), float(dists.var())  # 总体方差
...
def _batch_eef_pv(env, q_batch: np.ndarray, a_batch: np.ndarray,
                  robot_id: int = 0, show_tqdm: bool = False,
                  desc: str = "EEF [p,v]") -> (torch.Tensor, torch.Tensor):
    """
    逐样本计算：
      p = EEF 世界系位置（MjData.site_xpos）
      v = Jp(q) @ a（线速度）
    返回 (p_tensor, v_tensor)，各为 (B,3) 的 torch.float32
    """
    assert q_batch.shape[0] == a_batch.shape[0]
    B = q_batch.shape[0]
    p_out = np.empty((B, 3), dtype=np.float32)
    v_out = np.empty((B, 3), dtype=np.float32)
    iterator = range(B)
    if show_tqdm:
        iterator = tqdm(iterator, desc=desc, leave=False)
    for i in iterator:
        # 位置
        p_out[i] = _eef_pos(env, q_batch[i], robot_id=robot_id)
        # 线速度
        Jp, _ = _eef_jacobians(env, q_batch[i], robot_id=robot_id)
        v_out[i] = (Jp @ a_batch[i])
    return torch.from_numpy(p_out), torch.from_numpy(v_out)
...
def main():
    ...
    params['use_precompute_pv'] = bool(USE_PRECOMPUTE_PV)
    params['use_pv_as_act_cond'] = bool(USE_PV_AS_ACT_COND)
    print(params)
    ...
    # 动作条件键：若未提供则复用 obs 的键（注意：稍后会被 [p,v] 覆盖，不再用 keys 抽切）
    ...
    # === cond 维度：分别计算 obs-cond 与 act-cond（act-cond 稍后强制 6） ===
    ...
    # act-cond 维度后续以固定 6 维覆盖
    # cond_act_dim = cond_act_dim_src

    # === 为 merged_cond_keys 建索引映射（按 env.observation 的拼接顺序） ===
    # 训练阶段每条观测的结构： [ robot_obs | merged_cond_keys逐段拼接 ]
    # 我们需要从中抽出两份：c_obs（按 cond_obs_keys 顺序）与（备用）c_act（按 cond_act_keys 顺序）
    ...
    if USE_PRECOMPUTE_PV:
        # 使用“训练时的 obs_keys”（robot_obs_keys + merged_cond_keys）
        src_obs_keys_for_loading = src_train_obs_keys
        tgt_obs_keys_for_loading = tgt_train_obs_keys

        episodes_src_aug = _precompute_pv_and_augment_episodes_from_dir(
            episodes_dir=params['src_buffer'],
            obs_keys_for_loading=src_obs_keys_for_loading,
            ...
            capacity=None,
            tqdm_desc="[SRC] append [p,v] to obs"
        )
        episodes_tgt_aug = _precompute_pv_and_augment_episodes_from_dir(
            ...
            capacity=None,
            tqdm_desc="[TGT] append [p,v] to obs"
        )

        # buffer 的 obs_shape = 训练 env 的 obs 形状 + PV_DIM（因为我们在尾部追加了 [p,v]）
        src_obs_shape = (src_env.observation_space.shape[0] + PV_DIM,)
        tgt_obs_shape = (tgt_env.observation_space.shape[0] + PV_DIM,)
        ...
        # 回退路径：按照你原来的 keys 装（会在训练时在线算 [p,v]）
        src_buffer = replay_buffer.ReplayBuffer(
    ...
    # ------------------------------
    # Aligner（把 diffusion/flow 与 cond 传入）
    # ------------------------------
    ...
    # ======= 关键修订：动作通道先验 cond_dim 固定为 6（[p,v]） =======
    diffusion_act_bundle_cfg = {
        ...
        "cond_dim": int(COND_ACT_DIM_FIXED),   # ← 固定 6（[p,v]）
        "cond_keys": list(cond_act_keys),      # （仅日志）
    }

    # ============ Flow Matching Priors（来自预训练目录） ============
    ...
    # ------------------------------
    # 训练循环（注意：buffer.obs 可能带 [p,v]；aligner.update_* 仍喂 robot_obs）
    # ------------------------------
    ...
    for step in range(start_step, end_step):
        ...
        # cond(obs)：按 cond_obs_keys 顺序拼出来（保持原有观测先验不变）
        def _gather_cond(obs_full: torch.Tensor, robot_len: int, cond_slices: dict, keys: list):
            if not keys:
                return None
            parts = []
        ...
        # ------ NEW: act-cond = [p(3), v(3)] (B,6) ------
        if USE_PV_AS_ACT_COND:
            if USE_PRECOMPUTE_PV:
                # 直接从 obs 尾部读（我们在装载时已经把 [p,v] 追加在尾部）
                c_src_act = src_obs_full[:, -PV_DIM:].to(device)  # (B,6)
                c_tgt_act = tgt_obs_full[:, -PV_DIM:].to(device)  # (B,6)
            else:
                # 兼容：不预计算就在线算
                ...
                show = (step % PV_TQDM_EVERY == 0)
                p_src, v_src = _batch_eef_pv(src_probe_env, q_src, a_src, robot_id=0,
                                             show_tqdm=show, desc=PV_TQDM_DESC + " [src]")
                p_tgt, v_tgt = _batch_eef_pv(tgt_probe_env, q_tgt, a_tgt, robot_id=0,
                                             show_tqdm=show, desc=PV_TQDM_DESC + " [tgt]")
                c_src_act = torch.cat([p_src, v_src], dim=1).to(device)   # (B,6)
                c_tgt_act = torch.cat([p_tgt, v_tgt], dim=1).to(device)   # (B,6)
        else:
            # 兜底（不建议）：沿 keys 切片
        ...
        if step % params.get('log_freq', 1000) == 0:
            if USE_PV_AS_ACT_COND and (c_src_act is not None):
        ...
        if step % params['evaluation']['interval'] == 0:
            ...
            # === 仅在 Reach 任务下：记录最后一帧 target_to_robot0_eef_pos 距离到 TensorBoard ===
            if str(params.get("env_name", "")).lower() == "reach":
                # 这里复用同样的评估轮数；你也可以单独给一个更小的 episodes 数以加快评估
